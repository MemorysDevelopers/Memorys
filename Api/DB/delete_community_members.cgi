...
# 指定ユーザーのトレンド情報を削除する
def DeleteCommunityMembers(userId):
  sql = ''
  
  try:
    # 対象ユーザーのコミュニティメンバー情報を削除する
    cur = con.cursor()
    sql = "delete from community_members where member_id = '" + userId + "' "
...
try:
  ...
  try:
    ...
    # 指定ユーザーのコミュニティメンバー情報を削除する
    DeleteCommunityMembers(userId)
